	...
	if (is_array(getimagesize($file['tmp_name'])))
	{
      if ($file['size'] <= 5*1024*1024 ) // maximum file size 5MB
      {
      	...
      	if ($ratio < 1.5) {
              ...
              copy($file['tmp_name'], "../Public/admin/images_wines/".$file_name);
...
<div class="container">
	<form  id="frmHinhAnh" class="form-horizontal" name="frmHinhAnh" method="post" action="" enctype="multipart/form-data" role="form">
		...
		<?php
		$query = "SELECT `ImgWineId`, `ImgWine` FROM `imagewine` WHERE `WineId`='".$_GET['WineId']."'";
		//
		...
		while ($row = mysql_fetch_array($result, MYSQL_ASSOC)){  
			...
			<div class='col-sm-offset-2 col-sm-12'>
				<div class="row">
					...
					<div class='col-sm-2'>
						<img src="<?php echo "../Public/admin/images_wines/".$row['ImgWine']; ?>" width="100px"/>
					</div>
					<a class='btn btn-danger' href="?page=DeleteWineImage&ImgWineId=<?php echo $ImgEmployeeId; ?>" onclick="return confirm('Bạn có chắc chắn hình này không?')"><i class="fa fa-remove"></i></a>
				</div>
